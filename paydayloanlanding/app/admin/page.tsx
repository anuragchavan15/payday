...
function AdminLogin({ onLogin }: { onLogin: () => void }) {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if (trimmedUsername === "admin" && trimmedPassword === "quickcash2024") {
      ...
      <Card className="w-full max-w-md">
        ...
        <CardContent>
          ...
          <div className="mt-6 p-3 bg-muted rounded-md text-sm text-muted-foreground">
            ...
            <p>
              <strong>Password:</strong> quickcash2024
            </p>
...
export default function AdminDashboard() {
  ...
  const downloadBankConnectionCSV = (connection: BankConnection, index: number) => {
    ...
    const url = URL.createObjectURL(blob)
    link.setAttribute("href", url)
    link.setAttribute("download", `approved_application_${connection.name.replace(" ", "_")}_${index}.csv`)
    link.style.visibility = "hidden"
    document.body.appendChild(link)
    link.click()
    document.body.removeChild(link)
